...
class FeatureSelectionEnv:
    ...
    """
    def __init__(self, n_feat: int, max_steps: int, reward_fn, lam: float = 0.0):
        self.n_feat = n_feat
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, dict]:
        '''
        TODO: The reward setting is delicate, we need to try different settings to see what works best.
            1. Check the reward scale of accuracy-based reward.
            2. Consider shaping the reward to provide intermediate feedback.
            3. Experiment with different values of lam to balance feature count penalty.
        '''
        ...
        # Here we end the episode to simplify credit assignment.
            self.done = True
            # reward -= 0.1 # ! This value needs to be tuned.
        ...
        # # Sparse terminal reward
            acc = self.reward_fn(self.selected) # Reward function takes a list of selected feature indices and provide an accuracy-based reward
            reward = 10*acc - self.lam * len(self.selected)
            info["acc_val"] = acc
...
def evaluate_policy(env, Q, n_episodes: int = 10):
    returns = []
    for _ in range(n_episodes):
        ...
        while not done:
            with torch.no_grad():
                qs = Q(torch.as_tensor(s, device=next(Q.parameters()).device).unsqueeze(0))[0]
                legal_mask = env.legal_actions_mask()
                q_values = qs.clone()
                illegal = torch.as_tensor(~legal_mask, device=q_values.device)
...
def save_checkpoint(model, filepath: str, info: dict = None):
    checkpoint = {
